...
class Blockchain:
    ...
    def create_block(self, typer,list_trans, blkid, timestamp, version, creator, block_num, prev_hash=None):
        ...
        new_block = block.Block(typer,prev_hash, list_trans, blkid, timestamp, version, creator, block_num)
    ...
    def visualize_chain(self):
        ...
        for i in range(0, self.highest_block_num + 1):
            if i in self.blockchain.keys():
                ...
                print("Height = ",i,end=" - ")
                for blk in li:
                    print(blk.header['blkid'], blk.header['block_num'])
                    print("2nd Dimensional Blks",end="-")

                    if blk.header["block_num"]==0:
                        print("")
                        continue
                    new_chain = blk.header["2d_chain"]
                    for b in new_chain.keys():
                        print(new_chain[b].header['blkid'],end=" ")
                print("")
