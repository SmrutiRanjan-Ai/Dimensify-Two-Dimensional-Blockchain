...
class Node:
    ...
    def create_block(self, timestamp):
        ...
        if txn_list:
            ...
            status, new_block = self.blockchain.create_block("1d",blkid=new_blkid, list_trans=txn_list, \
    ...
    def add_block(self, timestamp, blkid, payout_delay=None):
        ...
        if block is not None:
            ...
            if block.header['blkid'] == blkid:
                ...
                new_blockchain={0:block}

                for i in range(10):

                    current_block_num=i
                    last_block=new_blockchain[i]
                    new_blkid = g.blkid_gen()
                    if self.mempool:
                        txn = self.mempool.pop()
                        txn_list = []
                    ...
                    type="2d"
                    prev_hash = last_block.header['block_hash']
                    status, new_block = self.blockchain.create_block(type,blkid=new_blkid, list_trans=txn_list, \
                                                                     timestamp=timestamp, version=g.VERSION,
                                                                     creator=self.id, block_num=current_block_num+1,
                                                                     prev_hash=prev_hash)
                    print("2d",status, new_block)
                    new_blockchain[i+1]=new_block
                block.header["2d_chain"]=new_blockchain
